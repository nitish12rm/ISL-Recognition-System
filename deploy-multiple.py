...
async def upload_videos(files: list[UploadFile] = File(...)):
    """
    Uploads multiple video files, processes them, and predicts the actions made in each video.
    Returns: List of predicted actions.
    """
    predictions = []
    ...
    for file in files:
        ...
        try:
            ...
            print(f"Processed numpy array shape for {file.filename}: {out_np_array.shape}")

            # Validate input shape for the model
            if out_np_array.shape != (45, 258):
                raise ValueError(
                    f"Unexpected input shape for {file.filename}: {out_np_array.shape}. Expected: (45, 258)."
                )
